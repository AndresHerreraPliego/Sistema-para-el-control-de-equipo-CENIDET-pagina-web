<template> 
    <v-sheet class="pa-4" height="100%" width="100%">
        <v-sheet class="overflow-y-auto overflow-x-hidden"  height="95%">
            ...
            <v-card-title  class="font-weight-bold text-body-2">Mi actividad en el sistema</v-card-title>
            <v-timeline dense>
                <v-timeline-item class="text-body-2" v-for="(item,i) in getitems('actividad').reverse()" :key="i"> 
                    <v-row>
                        <v-col cols="4">
...
export default {
    ...
    actividad: function(){
      ...
      this.getitems('actividad').forEach( item => {
        data.push( [item.tipo, item.descripcion, item.fecha, item.hora] )
      }); 
      return data
